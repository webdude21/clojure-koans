...
(def comma-join (partial str/join ", "))
...
(meditations
  ...
  (= "Rich Hickey aka The Clojurer aka Go Time aka Lambda Guru"
     (let [[first-name last-name & aliases]
       ...
       (str (format "%s %s aka ", first-name last-name) (str/join " aka " aliases))))
  ...
  (= "123 Test Lane, Testerville, TX"
     (let [{street-address :street-address, city :city, state :state} test-address]
       (comma-join [street-address city state])))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [[first-name last-name] {:keys [street-address city state]}]
        (comma-join [(str first-name " " last-name) street-address city state]))
